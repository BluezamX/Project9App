...
namespace App1.Managers
{
    class ConversionManager
    {
        ...
        public static List<Card> ConvertScryFallCard(List<ScryFallCard> scryfall)
        {
            ...
            {
                newCard.Add(new Card(card.name, card.type_line, card.set, card.oracle_text, card.flavor_text, card.artist, card.collector_number, new Uri(card.image_uris.png), card.mana_cost, card.toughness, card.power, card.loyalty, card.id));
            }
            return newCard;
        }
